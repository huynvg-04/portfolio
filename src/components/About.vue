...
onMounted(() => {
  ...
  sectionRef.value?.querySelectorAll('.fade-up, .fade-left, .fade-right, .fade-blur').forEach(el => observer.observe(el));
});
</script>
...
.tech-item:hover {
  ...
  color: var(--text-main);
  box-shadow: 0 0 12px color-mix(in srgb, var(--tech-color) 30%, transparent);
